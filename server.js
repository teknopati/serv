...
// 💾 Bellek İçi M3U Önbellek Sistemi
const m3uCache = {
    'tv.m3u': { mtime: 0, data: [] },
    'movie.m3u': { mtime: 0, data: [] },
    'series.m3u': { mtime: 0, data: [] }
};
...
function readM3UFile(fileName) {
    try {
        ...
        if (m3uCache[fileName] && m3uCache[fileName].mtime === stats.mtimeMs) {
            return m3uCache[fileName].data;
        }
        ...
        lines.forEach(line => {
            ...
            if (line.startsWith('#EXTINF:')) {
                ...
                let parsedDuration = durationMatch ? parseInt(durationMatch[1], 10) : -1;
                ...
                if (seasonInGroup) season = parseInt(seasonInGroup[1], 10);
                ...
                let durationInSeconds = 11 * 60; // Varsayılan süre
                if (parsedDuration > 0) {
                    durationInSeconds = parsedDuration;
                } else {
                    const nameLower = name.toLowerCase();
                    if (nameLower.includes("özel") || nameLower.includes("noel") || nameLower.includes("part") || nameLower.includes("movie")) {
        ...
        m3uCache[fileName] = { mtime: stats.mtimeMs, data: items };
        return items;
    } catch (e) {
        console.error(`[M3U Hata] ${fileName} okunamadı:`, e.message);
        return [];
    }
}

// 🔤 ALFABE KATEGORİ TANIMLARI
...
function getSeriesLiveState(seriesNameTarget) {
    ...
    let currentLoopPos = nowInSeconds % (totalDuration || 1);
...
app.get('/player_api.php', (req, res) => {
    ...
    if (!action) {
        return res.json({
            ...
            server_info: { url: req.hostname, port: PORT.toString(), https_port: "443", server_protocol: "http" }
        });
    }
    ...
    if (action === 'get_live_streams') {
        ...
        uniqueSeries.forEach((series, index) => {
            ...
            const channelName = (liveState && liveState.episode) ? `${series.name} 7/24 (${liveState.episode.name})` : `${series.name} 7/24`;
    ...
    if (action === 'get_series') {
        const uniqueSeries = getAllUniqueSeries();
        const seriesList = uniqueSeries.map((data, index) => ({
            num: index + 1,
            name: data.name,
            series_id: index + 1,
            cover: data.logo,
            plot: `${data.name} Arşivi`,
    ...
    if (action === 'get_series_info') {
        const seriesItems = readM3UFile('series.m3u');
        const targetId = parseInt(series_id, 10) || 1;
        const uniqueSeries = getAllUniqueSeries();
        ...
        targetEpisodes.forEach((ep, index) => {
            ...
            episodesObj[seasonKey].push({
                ...
                title: ep.name,
                container_extension: ext,
                info: { duration: `${Math.round(ep.durationInSeconds / 60)} dk`, plot: ep.name, movie_image: ep.logo || targetSeries.logo }
            });
        });
...
app.get('/:type/:user/:pass/:id', async (req, res) => {
    ...
    const cleanId = parseInt(cleanIdMatch[1], 10);
    let targetPath = null;
    ...
    // 3. Filmler
    if (!targetPath && cleanId >= 1001 && cleanId < 2000 && movieItems[cleanId - 1001]) {
    ...
    if (targetPath.includes('drive.google.com') || targetPath.includes('googleusercontent.com')) {
        ...
        if (driveId) {
            ...
            try {
                ...
                return res.status(500).send("Drive stream hatası.");
            }
        }
    }

    // Harici HTTP / Archive.org Akışları
    if (targetPath.startsWith('http://') || targetPath.startsWith('https://')) {
        return res.redirect(302, targetPath);
    }

    // Yerel Modem / Flash Bellek Yolu (volume(sda1)/...)
    let cleanPath = targetPath.replace(/^(ftp:\/\/|http:\/\/|smb:\/\/)?([^\/]+@)?[^\/]+\//, '');
    if (!cleanPath.startsWith('/')) cleanPath = '/' + cleanPath;

    return res.redirect(302, `http://192.168.1.1${cleanPath}`);
});

app.listen(PORT, () => console.log(`Xtream Proxy Sunucusu ${PORT} portunda aktif.`));
